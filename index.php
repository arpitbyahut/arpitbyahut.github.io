...
// --- Configuration ---
const BOT_TOKEN = '8492133869:AAFRr2aGQ0-JZzjnKyAgZYHbEw8GQEYgTHY';
const ADMIN_CHAT_ID = '5189651981';
const WEBHOOK_URL = 'https://sheinfaltu.infinityfree.me/index.php';
const CHANNEL_IDS = ['-1003427884031'];
...
function autoGenerateSheinAccount($chat_id, &$userData)
{
    ...
    if (!isset($userData['shein_accounts']))
        $userData['shein_accounts'] = [];
...
function getinsta($input, $creatorToken, $adId, $ip, $mode = 'connect')
{
    ...
    preg_match('/"actorID"\s*:\s*"(\d+)"/', $res, $m);
    $av = $m[1] ?? '';
    preg_match('/"rev"\s*:\s*(\d+)/', $res, $m);
    $rev = $m[1] ?? '';
    preg_match('/"haste_session"\s*:\s*"([^"]+)"/', $res, $m);
    $hs = $m[1] ?? '';
    preg_match('/"hsi"\s*:\s*"([^"]+)"/', $res, $m);
    $hsi = $m[1] ?? '';
    preg_match('/"f"\s*:\s*"([^"]+)"/', $res, $m);
    $fb_dtsg = $m[1] ?? '';
    preg_match('/jazoest=(\d+)/', $res, $m);
    $jazoest = $m[1] ?? '';
    preg_match('/"LSD",\[,\{"token":"([^"]+)"\}/', $res, $m);
    $lsd = $m[1] ?? '';
    preg_match('/"__spin_r"\s*:\s*(\d+)/', $res, $m);
    $spin_r = $m[1] ?? '';
    preg_match('/"__spin_t"\s*:\s*(\d+)/', $res, $m);
    $spin_t = $m[1] ?? '';
    preg_match('/"experience_id"\s*:\s*"([^"]+)"/', $res, $m);
    $experience_id = $m[1] ?? '';
...
function fetchVoucherWithRetry($creatorToken, $ip, $maxRetries = 30, $retryDelay = 4)
{
    ...
    for ($attempt = 0; $attempt < $maxRetries; $attempt++) {
        ...
        if ($voucherData && !empty($voucherData['voucher_code']))
            return $voucherData;
        if (!empty($vouchers) && !empty($vouchers[0]['voucher_code']))
            return $vouchers[0];
